...
def get_temp_email():
    ...
    for attempt in range(3):
        try:
            dom_res = requests.get("https://api.mail.tm/domains", headers=headers, timeout=10)
            ...
            username = "user_" + ''.join(random.choices("abcdefghijklmnopqrstuvwxyz0123456789", k=8))
            email = f"{username}@{domain}"
            password = "Password123!"
            
            acc_payload = {"address": email, "password": password}
            acc_res = requests.post("https://api.mail.tm/accounts", headers=headers, json=acc_payload, timeout=10)
            
            if acc_res.status_code == 201:
                token_res = requests.post("https://api.mail.tm/token", headers=headers, json=acc_payload, timeout=10)
...
def submit_registration(email, ua):
    ...
    challenge_payload = {
        "endpoint": "/gfxcafe.oku.account.v2.WaitlistService/RequestMagicLink",
        "clientInfo": {
            "userAgent": ua
        },
        "email": email
    ...
    try:
        challenge_req = requests.post(URL_GET_CHALLENGE, headers=headers, json=challenge_payload, timeout=10)
    ...
    try:
        response = requests.post(URL_REQUEST_MAGIC_LINK, json=payload, headers=headers, timeout=10)
        if response.status_code == 200:
            print("[+] Berhasil submit form!")
            return True
        else:
            print(f"[-] Gagal submit. Code: {response.status_code}, Res: {response.text[:200]}")
            return False
    except Exception as e:
...
def verify_email(token, ua):
    ...
    for _ in range(12):
        time.sleep(5)
        try:
            res = requests.get("https://api.mail.tm/messages", headers=headers_mail, timeout=10)
            if res.status_code == 200:
                messages = res.json().get('hydra:member', [])
                if len(messages) > 0:
                    msg_id = messages[0]['id']
                    
                    msg_res = requests.get(f"https://api.mail.tm/messages/{msg_id}", headers=headers_mail, timeout=10)
                    if msg_res.status_code == 200:
                        ...
                        if verify_link:
                            ...
                            conf_res = requests.get(verify_link, headers={"User-Agent": ua}, timeout=10)
                            print(f"[+] Status Respons Verifikasi: {conf_res.status_code}")
...
if __name__ == "__main__":
    ...
    for i in range(1, total_ref + 1):
        ...
        if not email_addr:
            print("[-] Gagal total membuat email temporer setelah 3x percobaan, melewati...")
            continue
            
        print(f"[+] Email Target: {email_addr}")
        
        if submit_registration(email_addr, ua):
            verify_email(mail_token, ua)
        
        if i < total_ref:
            print("[*] Menunggu 3 detik sebelum lanjut ke akun berikutnya...")
            time.sleep(3)
